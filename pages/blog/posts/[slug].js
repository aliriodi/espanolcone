          ...
          <article className="text-xs w-full md:w-3/4">
            <div className=" border-2 text-black bg-white md:rounded-lg overflow-hidden p-4 md:p-8">
              <div className='grid grid-cols-1  w-full sm:grid-cols-1 md:grid-cols-1  gap-4'>
                ...
                {locale === 'en' ? <h1 className="flex items-center justify-center text-gray-600 h-full text-5x5 font-bold mb-8">{en?.title}asdadas</h1> : null}
                {locale === 'pt' ? <h1 className="flex items-center justify-center text-gray-600 h-full text-5x5 font-bold mb-8">{pt?.title}asdadsa</h1> : null}
...
export async function getStaticPaths(locale) {
  ...
  aux.push(false);
    ...
    const paths = aux.flatMap(locale => {
      if (locale !== false) {
        return slugs.map(slug => ({ params: { slug: locale.join('/').join(slug) } }));
      } else {
