...
function Navbar(props) {
  ...
  function handleOnChange(lang) {
    if (props.slug) {
      ...
      //alert(lang.value)
      push(
        `/blog/posts/${props.slug}`,
        `/${lang.value}/blog/posts/${props.slug}&source=blog`,
        { locale: lang.value }
      );
  ...
  const languages2 = [
    {
      ...
      label: "ESPAÑOL",
      image:
        "https://res.cloudinary.com/dfddh08q8/image/upload/v1694363931/images/sp_tmnhok.jpg",
    },
    {
      value: "en",
      label: "INGLÉS",
      image:
        "https://res.cloudinary.com/dfddh08q8/image/upload/v1694363928/images/en_pybv0j.jpg",
    },
    {
      value: "pt",
      label: "PORTUGUÉS",
      image:
        "https://res.cloudinary.com/dfddh08q8/image/upload/v1694363923/images/br_qj0a0o.jpg",
    },
  ];

  const [language, setLanguage] = useState(
    languages2.find((objeto) => objeto.value === locale)
  );
  ...
  return (
    <>
      ...
      <nav className={`${styles["navbar"]} ${lightNavBar && styles["light"]}`}>
        ...
        <Link href="/">
          <Image
            ...
            src={
              lightNavBar
                ? "https://res.cloudinary.com/dfddh08q8/image/upload/v1694520319/images/logo-gradient_ssl8cl.png"
                : "https://res.cloudinary.com/dfddh08q8/image/upload/v1694366258/images/logo_etoaae.png"
            }
            alt="Español con E"
          />
        </Link>
        ...
        <div
          className={`${styles["navbar-menu"]} ${
            showMenuResponsive && styles["active"]
          }`}>
          {/* Botones Baner */}
          <ul
            className={`${styles["navbar-btns"]}`}
            style={
              language.label === "INGLÉS"
                ? { minWidth: "680px" }
                : { minWidth: "800px" }
            }>
            <li
              className={styles["select-languages"]}
              onClick={handleOnChangeLanguage}
              ref={scrollNavBar ? menuLanguage : null}>
              <div className={styles["select-languages_button"]}>
                ...
                {/* Label */}
                <label className={styles["select-languages_label"]}>
                  {t("labelLanguage")}
                </label>

                {/* Flecha del Responsive */}
                <FontAwesomeIcon
                  icon={showMenuLanguage ? faCaretUp : faCaretDown}
                  className={styles["select-languages_icon"]}
                />
              </div>

              {/* Menu Desplegable */}
              <ul
                className={`${styles["select-languages_menu"]} ${
                  showMenuLanguage && styles["active"]
                }`}>
                {languages2.length > 0 &&
      ...
      {/* Nav Bar Scroll */}
      <nav
        className={`${styles["navbar"]} ${lightNavBar && styles["light"]} ${
          styles["scroll"]
        } ${scrollNavBar && styles["active"]}`}>
        {/* Logo */}
        <Link href="/">
          <Image
            className={styles["navbar-logo"]}
