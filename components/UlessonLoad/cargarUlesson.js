    ...
    introducirVariable('Introduce el nombre de la unidad Microclase, example: Unidad 1 :  ', (unit) => {
      ...
      introducirVariable('Introduce una descripcion Recuerda que debe ser unica Microclase: ', async (description) => {
        ...
        introducirVariable('Introduce tags sepaarados por ",": ', async (tag) => {
          ...
          introducirVariable('Introduce topics o topico ciencia, teoria",": ', async (topics) => {
            // Hacer algo con la tercera variable

            console.log('',);

            introducirVariable('Introduce formats o tipo de articulo video,articulo, cancion",": ', async (formats) => {
              // Hacer algo con la tercera variable
              ...
              await imprimir(namefile, level, unit, description, tags, topics, formats)
              // Cerrar la interfaz de lectura
              rl.close();
            });
          });
...
async function imprimir(namefile, level, unit, description, tags,topics,formats) {
